        ...
        try:
            if method.lower() == 'get':
                ...
                if content_type == "multipart/form-data":
                    result = self.session.post(url=url, files=files, headers=self.headers, timeout=60).json()
                elif content_type == "application/json":
                    result = self.session.post(url=url, json=params, headers=self.headers, timeout=60).json()
                else:
                    result = self.session.post(url=url, data=params, headers=self.headers, timeout=60).json()
            else:
